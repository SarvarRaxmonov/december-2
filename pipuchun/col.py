...
async def  botquery(callback_query_data: types.CallbackQuery):
        ...
        if check_sub_channel(await bot.get_chat_member(chat_id=CHANNEL_ID, user_id=callback_query_data.from_user.id)) & check_sub_channel(await bot.get_chat_member(chat_id=CHANNEL_ID2, user_id=callback_query_data.from_user.id)):
                ...
                qorisura = {'1':{'r':nav.sura5, 'qori':'Mishary bin Rashed Alafasy'},'2':{'r':nav.sura9, 'qori':'Abdul Rahman Al-Sudais'},'3':{'r':nav, 'qori':' Saud Al-Shuraim '}, '4':{'r':nav, 'qori':'Saad Al-Ghamidi'},'5':{'r':nav, 'qori':'Bandar Bin Abdulaziz Baleela'},'6':{'r':nav, 'qori':' Idris Akbar '},
                ...
                keyingiall = {'keyingi5':{'t':cd3,'r':nav.sura6},'keyingi6':{'t':cd5,'r':nav.sura7},'keyingi7':{'t':cd7,'r':nav.sura8},'keyingi8':{'t':cd,'r':nav.sura9},'keyingi9':{'t':cd3,'r':nav.sura10},'keyingi10':{'t':cd5,'r':nav.sura11},'keyingi11':{'t':cd7,'r':nav.sura12},'ortga5':{'t':qorilist,'r':nav.qorilists},f'ortga{6}':{'t':cd,'r':nav.sura5},'ortga7':{'t':cd3,'r':nav.sura6},'ortga8':{'t':cd5, 'r':nav.sura7}
                             , 'ortga9':{'t':qorilist,'r':nav.qorilists},f'ortga10':{'t':cd,'r':nav.sura9},'ortga11':{'t':cd3,'r':nav.sura10},'ortga12':{'t':cd5, 'r':nav.sura11}
                              }
                for kenn , valu in keyingiall.items():
                        if  callback_query_data.data == f'{kenn}':
                              d = valu['t']
                              await callback_query_data.message.edit_text(d, reply_markup=valu['r'])
